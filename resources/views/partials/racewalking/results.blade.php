{{--
https://racewalk.com/profiles/stellaCashman.php
    https://racewalk.com/profiles/elliotDenman.php
    https://racewalk.com/profiles/daveRomansky.php
    https://racewalk.com/profiles/jakeJacobson.php
    https://racewalk.com/profiles/martinRudow.php
    https://racewalk.com/profiles/larryYoung.php
    https://racewalk.com/profiles/jackMortland.php
    https://racewalk.com/profiles/martinRudow.php

    https://racewalk.com/ORW/ohioRaceWalker.php
 --}}
 <x-card class="card-racewalk" nav=true>
        <h2>My Personal Results</h2>
        <div class="clear">
            <p><img class="float-left mr-4 mb-1" src="/images/racewalking/rw.bib.gif" alt="" width="193" height="163">Here are my race results, from my training logs, over the first 10 to 12 years that I actively raced. I started racing in early 1987 and petered out around 1997</p>

            <p>Isn't the web wonderful. Any goof ball can have a web site and post totally useless information ;-). Here for your entertainment ;-)</p>

            <p>Now if I can just start on the second ten year build up ;-)</p>

            <p>( <b>June, 2022 UPDATE</b> ) FInally getting back into walking.</p>
            <p>( <b>April, 2019 UPDATE</b> ) OOPS!!! That last ten years didn't go as planned. Here we go a gain.</p>
            <p>( <b>December 28, 2008 UPDATE</b> ) Officially start the next 10 years of race walking.</p>

            <p>Lots of the comments where made when I entered these results in my training log and some additional comments when I entered them here on the web. Some I don't understand myself.</p>
            <p class="test-center">[
            <a href="#results-2022">2022</a> | <a href="#results-2021">2021</a> |
            <a href="#results-2013-to-2020">2013 to 2020</a> |<a href="#results-2012">2012</a> |
            <a href="#results-2011">2011</a> | <a href="#results-2010">2010</a> | <a href="#results-2009">2009</a> | <a href="#results-2008">2008</a> | <a href="#results-1998">1998</a> | <a href="#results-1997">1997</a> | <a href="#results-1996">1996</a> | <a href="#results-1995">1995</a> | <a href="#results-1994">1994</a> | <a href="#results-1993">1993</a> | <a href="#results-1992">1992</a> | <a href="#results-1991">1991</a> | <a href="#results-1990">1990</a> | <a href="#results-1989">1989</a> |<a href="#results-1988">1988</a> | <a href="#results-1987">1987</a> ]
            </p>
        </div>
</x-card>

<a name="results-2022"></a>
<x-card class="card-racewalk">
    <h2 class="">2022</h2>
    <p>Back in the saddle again and hopefully this will stick. Getting back into my 1996-96 racing and training weight and getting in some walks.</p>
</x-card>

<a name="results-2019-2020"></a>
<x-card class="card-racewalk">
    <h2 class="">2019-2020</h2>
    <p class=""><b>COVID-19</b>. Not a good excuse for 2020, but onto 2021.</p>
    <p class=""><b>New age group January 2, 2020 &mdash; 65-69</b>. Perfect time to get back into it.</p>
</x-card>

<a name="results-2013-to-2019"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>2009 to 2018</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <h2 class="">2013 to 2018</h2>
            <p class=""><i>NO Races / NO training.</i> &bull; These 10 years, decades are flying by.</p>
            <h2 class="">2012</h2>
                <p class=""><em>Back training and plan on a few races this year. Races are my way of justifying training and a good benchmark for how the training is going.</em></p>
            <a name="results-2009"></a><a name="results-2010"></a><a name="results-2011"></a>
            <h2 class="">2009 | 2010 | 2011</h2>
                <p class=""><b>New age group January 2, 2009 - 55-59</b></p>
                <p class=""><em>Very limited training and no races for these few years.</em></p>
        </div>
</x-card>

<a name="results-2008"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>2008</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div class="race-result grid grid-cols-9 gap-2">
            <div class="race-date col-span-1">
                <p>12/28/1995</p>
            </div>
            <div class="race-name col-span-2">
                <p><b>Polar Bear Classic 10 Mile</b></p>
            </div>
            <div class="race-location col-span-1">
                <p>Asbury Park, NJ</p>
            </div>
            <div class="race-time col-span-1">
                <p>1:51:49</p>
            </div>
            <div class="race-notes col-span-9">
                <p>Average pace: 11:10. First race in 10 years or so. First 5 miles: <b>55:56</b>, Second 5 miles: <b>55:54</b>. Pretty even pace.
                First race in 10 years and last chance to race in the 50-54 age group. 30 lbs heavier and 30 minutes slower.</p>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1999-2007"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1999 to 2007</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <p class=""><i>No Races. A little break missing my 45-50 age group. And hopefully not my 50-55 ;-)</i></p>
        </div>
    </details>
</x-card>

<a name="results-1998"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1998</b> @svg('calendar-alt', 'text-gray-300')</span>
            <p>A couple of USATF events.</p>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/??/1998</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>4:45:11</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>17 out of 43</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>11/23/1998</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>National One Hour Postal</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>11,716m</p>
                </div>
                <div class="race-notes col-span-4">
                    <p class="text-green-700"><b>3rd in 40-45, 10th in Men's Open</b></p>
                </div>
            </div>
        </div>
</x-card>

<a name="results-1997"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1997</b> @svg('calendar-alt', 'text-gray-300')</span>
            <p>The last of my busy years of Racewalking.</p>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/13/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn</a> 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ on the Boardwalk</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:35:16</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace: 9:32. 2nd - L7-9:30, L6-9:15, L5-9:09</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/26/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://www.thepennrelays.com/" target="_blank">Penn Relays</a> 5Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Franklin Field, Phadelphia, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>DNF</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>5 Km in 26:00</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/1/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>USATF-NJ Masters 5 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Tinton Falls, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:33</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st 40-44, John S.  23:29, Ray F. 25:29</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/16/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:53</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st - First race of season. 1 mile in 8:00</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/22/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Homdel International 1 Mile handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Homdel, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:18</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Beat John S. in sprint to tape. Great race. We both started scratch</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/23/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>27:20</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Easy race - workout</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/25/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Shore AC Development Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Monmouth University, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:24</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st - 1:42, 3:34</p>
                </div>
            </div>

            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/30/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:43</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st - 1 mile in 8:05, 2 @ 16:15, 8:17 average</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/4/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Grand Old Flag 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Pottstown, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>55:17</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>First mile 8:40, Hot and Humid</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/14/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>28:14</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st - 1 mile in 9:34, 2 @ 9:05, 9:05 average</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/21/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:48</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/28/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>28:31</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st - Easy 40K pace for 2 laps with Jack and Ron</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>8/24/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Sallie Mae 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Wilkesbarre, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:20</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>9th HR avg 177</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/7/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <p><b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>DNF</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Hot day, 10K-59:30, 20K-1:58:59(57:29), 30K-</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/14/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Harry Drazin 5.2 Mile handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Interlaken, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>47:32</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Went out way to fast trying to catch Dave Romansky, 7:50 pace 1st lap, 9:08 average.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>10/13/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF National 2 hour Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Wochester, MA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>22,135m</p>
                </div>
                <div class="race-notes col-span-4">
                    <p class="text-red-500 font-extrabold">3rd overall - 2nd open medal.  (1:47:43 for 20Km)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>10/26/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Race For Hunger</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Freehold, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>41:30</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd - Stayed with Dave for first mile: 7:46, 8:10, 8:27, 8:29, 8:37</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>11/2/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>One Hour Postal</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Freehold, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>50:25</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd - Don't remember the meters.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/14/1997</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Todd Scully 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>52:33</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Round and round Lake Tak</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1996"></a>
<x-card class="card-racewalk" nav=true>
    <details open>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1996</b> @svg('calendar-alt', 'text-gray-300')</span>
            <p class=""><b>1996 was a very good year. PRs at a most distances.</b> <span class="text-green-700 font-extrabold">NJ Racewalker of the Year</span>. Even had a top 30 time for USA walkers in the 20km.</p>
        </summary>
        <div>
            <img class="img-thumbnail block mx-auto my-12" src="/images/racewalking/1996-US-20km-list.png" alt="">

            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>1/27/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K Winter Series</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:31</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Rainy and Cold</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>3/17/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>St Patty's 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Freehold, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:24:08</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace: 8:25. 1st, 8:25 average, L10-1 8:29, 9:00, 8:37, 8:25, 8:38, 8:18, 8:09, 8:25, 7:57, 8:07, * 5M at 41:00 Jack (Loch) 1:32</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>3/24/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>National Invitational 20 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    ????
                </div>
                <div class="race-time col-span-1">
                    <p class="font-black text-green-700">1:45:11</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - <b>15th</b>, 5:12, 5:21, 5:23, 5:17, 5:14, 5:11, 5:14, 5:10, 5:06, 5:16
                    5:11, 5:13, (52:42), 5:13, 5:15, 5:15, 5:17, 5:21, 15:52</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/12/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Ron Zinn 10 Miles</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p><span class="font-extrabold text-red-500">1:22:12</span>  (avg. pace: 8:13)</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st - 8:13 average, 1-7:45, 8:00, 8:01, 8:06, 8:13, 8:23, 8:23, 8:33, 8:22, 8:22 40:07 at 5 Miles </p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/27/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://www.thepennrelays.com/" target="_blank">Penn Relays</a> 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Franklin Field, Philadelphia, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p class="font-black text-green-700">49:16</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 3 (age group), First time under 50 minutes. Even pace 24:23 at 5K</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/5/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Al Saner Days 20K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Point Pleasant Beach, NJ </p>
                </div>
                <div class="race-time col-span-1">
                    <p class="font-black text-green-700">1:43:11</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 1st, L10-10:35, L9-10:32, L8-10:44, L7-10:21, L6-10:12,
                    L5:10:13, L4-10:08, L3-10:04, L2-10:09, L1-10:10. Average pace: 8:18</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/8/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>1 Hour Postal</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>????</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>10K in <span class="font-black text-green-700">47:49</span> (average: 7:42)</p>

                    <p><b class="font-extrabold text-green-700">Big PR</b> - Wanted to break 49 minutes before a fellow training partner did. So I used the postal to time-trial a 10 Km. L5-9:37, L4-9:43, L3-9:29, L2-9:30, L1-9:29, 5 K <b>23:44 5 Km <b class="font-extrabold text-green-700">PR</b>.</p>

                    <p>I wanted to stop after the 10Km and just take what ever the meters where for the hour but <a href="{{ config('racewalk.ElliotDenman.url') }}"><b>Elliot Denman</b></a>@svg('link') talked me into going on after a very short rest.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/19/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 10 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Niagra Falls, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p>50:45</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">9th overall</b>, 1st 40-44 (3rd Masters title), 80 degrees and windy, L1-9:47,
                    L2-10:03, L310:15, L410:24, L5-10:15</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/26/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Eastern Regional 20 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Central Park, NYC, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:45:55</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>3rd overall, 1st 40-44 L10-11:51, L9-11:25, L8-10:54, L7-10:43,
                    L6-10:30, L5-10:20 (10 Km 50:31), L4-10:13, L3/210:08, L1-9:40</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/2/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>NJ Masters Championship 5 Km (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Monmouth Regional HS, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p>23:52</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace: 7:41. 1st overall, 7:36, 7:33 (John Soucheck 2nd and Ray Funkhouser 4th).
                    This is the first time I've beaten Ray. Ray took it out hard and I reeled him in
                    with a few laps to go</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/10/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p class="font-black text-green-700">23:36 (7:36 average)</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 1st, 7:35, 15:19 (John S. 23:39)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/17/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>23:42</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, 7:38 average</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/24/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:32</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, 7:54 average.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/26/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Shore AC Development Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Monmouth University, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:05</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 1st 1:46 Avg, L4-1:40, L3-1:48[5:24], L2-1:51[3:36], 1:44</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/1/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:29</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Leg sore from building a deck over the weekend, took it easy-Mile 1 in 8:24, mile 2 in 8:15, last mile under 8:00</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/26/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Shore AC Development Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Monmouth University, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>6:49</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 1st 1:42 Avg, L4-1:38, L3-1:42.28, L2-1:45.25, 1:43.07</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/4/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Grand Old Flag 10 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Pottstown, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>51:17</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>L1-8:06, L2-7:56, L38:02, L4-9:04, L5-7:50, L6.2-10:21 - WORKOUT</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/8/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p class="font-black text-green-700">23:15</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - Mile 1 in 7:12, mile 2 in 7:32, last mile 7:43-7:29 average. Now if you could only do this for a 20 Km ;-). Average pace: 7:45</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/10/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Homdel International I Mile Handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Homdel, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p class="font-black text-green-700">6:48.85</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 1st, L1-1:34, L2-1:45, L3-1:47, L4-1:41</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/15/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:41</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Mile 1 in 7:47 (7:57 average)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/17/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Ocean HS Dev Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Oakhurst, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:27</p>
                </div>
                <div class="race-notes col-span-4">
                    <p class="text-xs text-gray">After 9pm day (Don't know what this entry means. Probably one of those days where I worked through the night programming and was up for 36 hours or more.). On of those development meet miles I went just to help and was in my street shoes (moccasins) and races after Elliott Denman talked me into it.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/22/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>23:15</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Mile 1 in 7:20, mile 2 in 7:30 (7:29 average)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/29/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>23:18</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Mile 1 in 7:20, mile 2 in 7:30 (7:29 average)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>8/5/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>23:33</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Mile 1 in 7:19, mile 2 in 7:33 (14:51) (7:34 average)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>8/12/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>23:26</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Mile 1 in 7:23, mile 2 in 7:29 (14:51) (7:34 average)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>8/19/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:26</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>8/26/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Sallie Mae 5 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Wilkes-Barre, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p><span class="font-extrabold text-green-700">23:09</span> (7:27 average)</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 1st 1K-4:27, 3K-14:09, Last K 4:34</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/2/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Harry Drazin 5.2 Mile Handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Interlaken, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>47:00</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>10th - 10:26, 20:28, 10:21, 10:22 (7:55 average)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/8/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p><span class="font-black text-red-700">3:59:38</span></p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd overall, <b class="font-extrabold text-green-700">PR</b> - Hot Day, Splits - 10K-59:30, 20K-1:56:59(57:29),
                    30K-2:54:38(57:49), 40K-(1:05:00), <b>Silver Medal, Open Division</b>, 2nd Masters (Warrick Yeager 1st)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/22/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Atlantic City 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Atlantic, NJ on the boardwalk</p>
                </div>
                <div class="race-time col-span-1">
                    <p>51:06</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>4th overall overall, 5K-24:49</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>10/27/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Race For Hunger</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Freehold, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>41:03</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Stayed with John S. for 3 miles</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/15/1996</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Todd Scully 10 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>55:06</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd - Stayed with John S. for 3/4 mile. Need major endurance training to get ready for March.</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1995"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1995</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/16/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn</a> 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:30:23</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>  (avg. pace: 9:00)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/29/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://www.thepennrelays.com/" target="_blank">Penn Relays</a> 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Franklin Field, Philadelphia, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>51:42</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>4th master, 7:55, 8:07, 7:59- 5K(25:07) 8:26, 8:??, 8:40</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/7/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Al Saner Days 20Km (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Point Pleasent, NJ </p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:48:30</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>8:44 average, <b class="font-extrabold text-green-700">PR</b> 2nd Track 10K in 53:57 13-8:56, 8:41, 8:38, 8:28, 8:30; 8:40, 8:37, 8:33, 8:42, 8:40 1-8:33 </p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/28/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <p><b>USATF 10 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Niagara Falls, NY </p>
                </div>
                <div class="race-time col-span-1">
                    <p>52:48 </p>
                </div>
                <div class="race-notes col-span-4">
                    <p>18th overall, 4th Master, <b>1st in age group (<b class="font-extrabold text-red-500">2nd National Masters title</b>)</b></p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/12/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:35</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Here we go again, round and round the lake</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/19/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:38</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/23/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Homdel International 1 Mile Handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Homdel, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p><b class="font-extrabold text-green-700">PR</b> - 7:07</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/26/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:23</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/3/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:35</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/4/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Firecraker 5 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Wall Township, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>42:54</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>5th, 3rd in age group</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/5/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Ocean HS Dev Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Oakhurst, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:28</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/10/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:48</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/12/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Ocean HS Dev Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Oakhurst, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:30</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/16/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>XI WAVA Games 20 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Niagria Falls, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:52:13</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="">
                <p><img class="float-left img-thumbnail mr-2" src="/images/racewalking/CS-20k-WAVA-1995.png" alt="" width="77" height="81">
                <b>XI WAVA Games 20 Km</b> - <b>24/300</b> Overall, <b>8/21</b> in 40-44 age group. 54:34 10K. <span class="font-extrabold text-green-700">Team Gold (USA) 40-44 Age Group</span>. 90 plus degrees. Went out slow and passed lots of walkers in the second half that went out too fast in the heat.</p>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/19/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Ocean HS Dev Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Oakhurst, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:13</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/22/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>XI WAVA Games 5Km (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Buffalo, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:34</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result">
                <p><img class="float-left img-thumbnail mr-2" src="/images/racewalking/CS-5k-WAVA-1995.png" alt="" width="54" height="81">
                    <b>XI WAVA Games 5Km (track)</b> <b>7th/22</b> - 1600 in 7:17, lead 2.5 laps. If you run in Dave Romanasky have him tell you the story of my 5 Km race here. Pretty entertaining as he tells it. Went out and lead the race for a few laps - too fast and one shouldn't PR you first mile.</p>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/26/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Ocean HS Dev Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Oakhurst, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:34</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/31/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>27:05</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Walked with Phillis (Pacing)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>8/7/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:25</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>3rd, 7:44-7:44 7:51, 7:51 Average </p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>8/14/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:31</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>3rd, 7:59-16:07</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/2/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Eatontown 5 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Eatontown, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:02</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st <b class="font-extrabold text-green-700">PR</b>, 7:44, 7:47 for Mile one</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/3/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Harry Drazin 5.2 Mile Handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Interlaken, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>41:56</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>4/30</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/10/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>4:26:00</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>7th, Great for 30K, sub 4 pace. slow last 10 K</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/17/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Sallie Mae 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Wilkes-Barre, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:20</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st (probably age group), Won $100 and beat Big Dave (Romansky)
                    by a few seconds, 1000 meters in 4:30
                    </p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/24/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Atlantic City 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Atlantic, NJ on the boardwalk</p>
                </div>
                <div class="race-time col-span-1">
                    <p>50:46</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>7th overall, 8:10 pace, 5K in 25:01, Dave under 50:00, won $50. First big
                    payday racewalking ;-)
                    </p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>11/5/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Regional 20K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Arlington, VA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:45:27</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>(8:29 pace), 4th, Lap 13-8:42, 8:34, 8:29, 8:23, 8:24, 8:29,(52:35),8:29, 8:32, 8:25, 8:33, 8:17, Lap 1-8:14. American Record 40-44 (soft record at the time, but an <b class="font-extrabold text-green-700">American Track Record</b>)
                    </p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>11/24/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Freehold 5 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Freehold, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p><span class="font-extrabold text-red-500">39:34</span></p>
                </div>
                <div class="race-notes col-span-4">
                    <p> (avg. pace: 7:55), 2nd, 7:29, 7:47, 7:57, 8:15, 8:04, 7:55 pace. Chasing Ray Funkhouser as usual.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/17/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Todd Scully 10 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>49:27</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>PR Stayed with John (Soucheck) for 5.5 Miles). First time cracking 50 minutes.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>????</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>26:15</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>8:20, 8:30</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/31/1995</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Polar Bear Classic 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p><b class="font-extrabold text-red-500">1:21:41</b></p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 3/20 1st 40-44, 7:35, 8:10, 8:07, 7:59, 8:08, 8:10, 8:07, 8:21, 8:36, 8:25, (avg. pace: 8:04)</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1994"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1994</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/10/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn</a> 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:42:58</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Avg. pace: 10:18.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/15/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Eastern Regional 20K</b></p>
                    </p>
                </div>
                <div class="race-location col-span-1">
                    <p>Wall Township Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:58:32</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Out sprinted by Jack Loch.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/13/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>26:48</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/22/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Homdel International 1 Mile handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Homdel, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:41</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/24/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>26:58</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/27/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>26:26</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/4/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>27:08</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/11/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>26:15</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, 8:20, 8:30</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/18/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:52</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, 7:53-18:30</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/4/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Harry Drazin 5.2 Mile handicap</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Interlaken, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>44:00</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>8:27, Good even pace. Training for 40 Km</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/11/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>4:06:12</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>PR - 10th, 3rd 40-44, 1st open team. 30K at 2:59:52. Good even pace race.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/18/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Todd Scully 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>54:33</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Around Lake Tak</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/27/1994</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Polar Bear Classic 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:33:42</p>
                </div>
                <div class="race-notes col-span-4">
                    <p><span class="font-extrabold text-green-700">1st</span>, Final sprint to beat Jack (Lach) in closet finish ever. And probably one of the slowest winning times for the race. (avg. pace: 9:22)</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1993"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1993</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/11/1993</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 10 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Niagria Falls, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:05:00</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>PW (Personal Worst)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/12/1993</p>
                </div>
                <div class="race-name col-span-2">
                    <p>
            <img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>

                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>4:34:37</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>14th, Team Bronze-Shore AC, Bill Norton 4:10</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/26/1993</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Polar Bear Classic 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:53:00</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Avg. pace: 11:18.</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1992"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1992</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/19/1992</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="float-left" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>5:05:00</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Too many DNFs. Goal was to finish. Team bronze.</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1991"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1991</b> @svg('calendar-alt', 'text-gray-300')</span>
            <p><b>2019 UPDATE</b>: Looks like 1990 to 1993 where slow years.</p>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>3/3/1991</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Bethesda Chase 20K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Bethesda, MD</p>
                </div>
                <div class="race-time col-span-1">
                    <p>2:05:00</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/8/1991</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>DNF</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Bailed with 2 laps to go.</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1990"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1990</b> @svg('calendar-alt', 'text-gray-300')</span>
            <p><b>2019 UPDATE</b>: One race in 1990 - Have to look back in the training logs or lack of training logs to see what happened.</p>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/9/1990</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Brookdale College, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>DNF</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Great weather but dropped at 32.5K in 3:33, 20K in 2:06, Bill Norton 4:08</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1989"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1989</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <h3 class="">1989</h3>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>1/22/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>MAC Indoor 1500 Meter</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Haverord, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>6:56.7</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/9/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn</a> 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ on the Boardwalk</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:31:57  </p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Avg. pace: 9:12 off very little training.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/16/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>National Invitational 20K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Washington, DC</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:55:28</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/29/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://www.thepennrelays.com/" target="_blank">Penn Relays</a> 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Franklin Field, Philadelphia, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>52:30</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/21/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Boardwalk 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Seaside Heights, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:25:17</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace 8:32. Dodging kids during senior week and chasing Ray Funkhauser.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/27/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Sandy Kalb 9 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Lakewood, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:11:46</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace 7:58. Twice around Lake Carassajo. Walked with <a href="https://racewalk.com/profiles/maryanneTorrellas.php" target="+blank"><b>Maryanne Torrelles</b> @svg('link')</a>. Fast walkers can sure make one faster. This was a good race.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/13/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 5K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:40</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, another year and more slugging it out with Bill Norton</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/27/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 5K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:56</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/4/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Grand Old Flag 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Pottstown, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>53:16</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Training race. 1st 5K easy, 2nd 5K hard. Hometown race along parade route. Most if not all never saw a racewalker.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/10/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 10 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Niagara Falls, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p><b>50:38</b></p>
                </div>
                <div class="race-notes col-span-4">
                    <p><b class="font-extrabold text-green-700">PR</b> - 1st age group, <b>National age group gold medal</b> (<b>1st National Masters Championship</b>) </p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/15/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Championship 5K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:47</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, Reading HS track. 105 track temp</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/10/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Brookdale College, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>DNF</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>20K in 2:10. This was a death march race with temps 90 plus.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>10/8/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Hanover 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Hanover, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>51:55</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>10/22/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Italian American 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Oakhurst, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>52:29</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd behind Ray Funkhouser, also 1 hour postal and forget the distance I got. Always seem to be chasing Ray.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>11/11/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Union Canal</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>51:01</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd, Bill Norton 1st</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>3/4/1989</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Bethesda Chase 20 Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Bethesda, MD</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:52:30</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd, Hilly, 1st mile in 8:05, need training.</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1988"></a>
<x-card class="card-racewalk" nav=true>
    <details>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1988</b> @svg('calendar-alt', 'text-gray-300')</span>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>1/6/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Philadelphia Indoor 1600 meters (Open)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Haverford, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:27.8</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st indoor race. Racing Bill Norton as usual and sucking air where there isn’t any.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>1/7/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Philadelphia Indoor 1600 meters (Masters)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Haverford, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:35.1</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Racing Bill Norton as usual.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>2/21/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Lake Tak 10 Miler</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Deal, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:24:12</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st place. Good even effort.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>3/6/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Bethesda Chase 20Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Bethesda, MD</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:52:25</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average page: 9:03. 5 out 22, Hilly course. 1st mile 7:50, 5K -25:51, 10K - 55:21 (8:35p), 10M - 1:28:07 (8:48p), last 2.4 (24:20)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>3/27/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>National Invitational 20K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Washington, DC</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:50:30</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace: 8:52. 3rd in age group, round and round the Lincoln Memorial Reflecting pool. This was one of the best settings for a race. This along with the Niagara Falls races.</p>
                    <p>Splits: 10Km 54:23 (8:45p) / 55:55 (9:00p)
                </div>
            </div>

            <h4>Ronn Zinn 10 Miler, Asbury Park boardwalk, NJ</h4>
            <div class="race-result">
                <div class="grid grid-cols-4 gap-4">
                    <div class="col-span-1">
                        <img class="img-thumbnail" src="/images/photos/racewalk/CS-Zinn-88.png"
                            alt="CS-Zinn-88.png">
                        <p class="caption mt-1">1:25:27 2nd place, 1988 <a href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn</a> 10 Miler, Asbury Park boardwalk, NJ.</p>
                    </div>
                    <div class="col-span-2">
                        <img class="img-thumbnail" src="/images/photos/racewalk/CS-ZINN-88(2).png"
                            alt="CS-ZINN-88(2).png">
                        <p class="caption mt-1">1988 Ronn Zinn 10 Miler, Asbury Park boardwalk, NJ.</p>
                    </div>
                    <div class="col-span-1">
                        <img class="img-thumbnail" src="/images/photos/racewalk/CS-ZINN-88(3).png"
                            alt="CS-ZINN-88(3)">
                        <p class="caption mt-1">Racing Edgardo Rodriguez</p>
                    </div>
                </div>
                <p class="text-center">1988 <a class="underline-brand" href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn <x-ui.icons.link /></a> 10 Miler, Asbury Park boardwalk, NJ. &bull;
                <p class="text-sm text-center"><i>Photos by <a href="{{ config('racewalk.ElliotDenman.url') }}"><b>Elliot Denman</b></a>@svg('link')</i></p>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/11/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn</a> 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ on the Boardwalk</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:25:27</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace: 8:33. 2nd and very windy.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/27/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://www.thepennrelays.com/" target="_blank">Penn Relays</a> 5Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Franklin Field, Philadelphia, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:50</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/19/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Great Valley Corporate Center 5Km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Malvern, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:33</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Training race. Jumped into a running road race and beat lots of runners.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/26/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:49</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, Racing your know who as usual.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/29/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Sandy Kalb 9 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Lakewood, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:18:07</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd out of 20, Two times around Lake Caeassajo. No water aide stations and a very hot day. average 180 spm for 4.5 miles (one loop) with <a href="https://racewalk.com/profiles/maryanneTorrellas.php" target="+blank"><b>Maryanne Torrelles</b> @svg('link')</a>.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/2/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 3K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>14:49</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, Racing your know who as usual.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/30/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>24:54</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, Racing your know who as usual.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/7/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 3K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>14:14</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st, Racing your know who as usual.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/9/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 10 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Niagara Falls, NY</p>
                </div>
                <div class="race-time col-span-1">
                    <p>51:51</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>109 walkers, age group 10/20</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/16/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Championship 5Km (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:16</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Undefeated season. 100 + track temp.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/27/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>All Comers Meet 1 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Seattle, WA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>7:19.1</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd, Allen James 1st. On a training trip for work to HQ of Aldus Pagemaker. Always great finding a race on the road. Was on a trip to Aldus (PageMaker DTP software) for work training.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/11/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Kenn College, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>DNF</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Out at 35K, Sub 4 hour pace at 30K (2:59) (a little too fast I guess)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>11/5/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Union Canal 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>55:19</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2nd, I don't always get the best of Bill Norton.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/31/1988</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Polar Bear Classic 10 Mile (9 Miles)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:20:59</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace: 9:00. 4th, doors where closed so the race was shortened by 1 mile to nine miles. And, one more mile and Bill would have caught me. Glad those doors were shut :-)</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<a name="results-1987"></a>
<x-card class="card-racewalk" nav=true>
    <details open>
        <summary class="mb-4">
            <span class="font-heading text-brand-500 text-3xl"><b>1987</b> @svg('calendar-alt', 'text-gray-300')</span>
            <p class="mt-4"><b>(33 years old)</b> - Stumbled onto racewalking some how. And really got into it via <b>Jake Jacobson's Healthwalk to Fitness</b> book. I found this book at the Chester County Library, Exton, PA USA and checked it out several times. <em>Thank You Jake</em>.</p>
        </summary>
        <div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>3/29/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>National Invitational 5km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Washington, DC</p>
                </div>
                <div class="race-time col-span-1">
                    <p>30:24</p>
                </div>
                <div class="race-notes col-span-4">
                    <p class="text-lg font-bold">First Official Race</p>
                    <p>Around the <a href="https://en.wikipedia.org/wiki/Lincoln_Memorial_Reflecting_Pool" target="_blank">Lincoln Memorial Reflecting Pool @svg('link')</a>. 1st race and I was 2 minutes late. Finished 13th out of 48. Would have been 2nd behind Bobby Briggs if I had showed up on time ;-)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>4/12/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b><a href="http://en.wikipedia.org/wiki/Ronald_Zinn" target="_blank">Ronn Zinn</a> 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ on the Boardwalk</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:44:14</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average pace: 10:25. Went down to check this race out and actually did the race. I read about the races on the boardwalk at Asbury Park in Howard "Jake" Jacobson's Healthwalk to Fitness book. Very cool racing on the boardwalk. The start of a lot of great racewalking friendships.</p>
                    <p class="">Meet <a href="//en.wikipedia.org/wiki/Elliott_Denman" target="_blank"><b>Elliott Denman</b> @svg('link')</a> and the great walkers of the Shore AC and started a long, long friendship with Elliot and many of the walkers. Elliot, born January 23, 1934 competed in the men's 50 kilometers walk at the 1956 Summer Olympics. I spent every Wednesday for several years on a long, long 4 hour plus, long and show training walk with Elliott and Dr. Gerald Kass while I lived in Asbury Park, NJ.</p>

                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>5/3/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Central Park 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Central Park, NYC</p>
                </div>
                <div class="race-time col-span-1">
                    <p>28:16</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>45 out of 135 - Staged with the 1987 World Cup in Central Park. Great time racing in central park and then watching the very best walkers in the world.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/11/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 3K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>15:14</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Racing Bill Norton every week.</p>
                </div>
            </div>

            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/18/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Reading Track Club Development Meet 5K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:48</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>6/25/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Pennsylvania Keystone Games qualifier 5K (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                </div>
                <div class="race-time col-span-1">
                    <p>26:44</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>
                <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>7/12/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Pennsylvania Keystone Games 5K Finals (track)</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Leigh Univ, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>28:55</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2/5 VERY hot. Never did pick up my medal.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/13/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Eastern Regional 10K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Atlantic City, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>57:28</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>On the boardwalk start and finish loops in front of the Atlantic City Convention Center.</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>9/20/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p>
                    <img class="" src="/images/racewalking/rw.USATF_sm.gif" alt="" width=29 height=26 border=0>
                    <b>USATF 40 Km National Championship</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Fort Monmouth, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>4:32:28</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>1st long distance race. Just wanted to finish. 5 Km loops and old course at Fort Monmouth, NJ.</p>

                    <p>Laps: 1) 34:15 2) 33:52 3) 33:20 4) 34:03 5) 33:25 6) 34:14 7) 33:13 8) 36:03 Average: 34:04 and 10:57 pace.</p>
                </div>
            </div>
            <div class="race-result">
                <p class="text-sm">I trained with <b>Bill Norton</b> who talked me into doing this race and gave me some great advice that helped me finish the race. Some advice I didn't take like the pre-race meal of Beer and Muttin chops. I could barely walk the next day and on Bill's great advice forced myself out for a walk. This does help in the recovery.</p>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>10/18/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Shore AC 10km</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>53:07</p>
                </div>
                <div class="race-notes col-span-4">
                </div>
            </div>

            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>10/25/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Union Canal 5K</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Reading, PA</p>
                </div>
                <div class="race-time col-span-1">
                    <p>25:31</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>2/12, Racing Bill Norton as usual (Bill 3rd)</p>
                </div>
            </div>
            <div class="race-result grid grid-cols-9 gap-2">
                <div class="race-date col-span-1">
                    <p>12/27/1987</p>
                </div>
                <div class="race-name col-span-2">
                    <p><b>Polar Bear Classic 10 Mile</b></p>
                </div>
                <div class="race-location col-span-1">
                    <p>Asbury Park, NJ</p>
                </div>
                <div class="race-time col-span-1">
                    <p>1:27:08</p>
                </div>
                <div class="race-notes col-span-4">
                    <p>Average page: 8:43. On the boardwalk</p>
                </div>
            </div>
        </div>
    </details>
</x-card>

<x-card class="card-racewalk" nav=true>
    <h2 class=""> No Racewalking Before 1987</h2>
</x-card>
